...
class TestUserCommands:
    ...
    def test_user_profile_success(self, mock_client_class, mock_asyncio_run, runner, mock_config):
        ...
        # Mock user data
        mock_user = AsyncMock(model_dump=lambda: {
            "id": 456,
            "name": "Test User",
    ...
    @patch("easel.cli.commands.user.asyncio.run")
    @patch("easel.cli.commands.user.CanvasClient")
    def test_user_courses_success(self, mock_client_class, mock_asyncio_run, runner, mock_config):
    ...
    @patch("easel.cli.commands.user.asyncio.run")
    @patch("easel.cli.commands.user.CanvasClient")
    def test_user_roster_success(self, mock_client_class, mock_asyncio_run, runner, mock_config):
    ...
    @patch("easel.cli.commands.user.asyncio.run") 
    @patch("easel.cli.commands.user.CanvasClient")
    def test_user_roster_with_role_filter(self, mock_client_class, mock_asyncio_run, runner, mock_config):
        ...
        with patch("easel.config.paths.get_config_dir", return_value=mock_config._config_dir):
            result = runner.invoke(cli, ["user", "roster", "123", "--role", "student", "--format", "json"])
